...
const SpecularButton: React.FC<SpecularButtonProps> = ({
  ...
  useEffect(() => {
    ...
    // Mobile / touch screen detection: skip heavy multiple WebGL contexts on phones
    // and rely on sleek hardware-accelerated CSS specular rim for 120 FPS buttery smoothness
    ...
    try {
      const dpr = typeof window !== 'undefined' ? 1 : 1; // DPR 1 for buttons is sharp & avoids fill-rate bottlenecks
      ...
      const resize = () => {
        ...
        const w = rect.width || btn.clientWidth || 120;
        const h = rect.height || btn.clientHeight || 40;
        sizeRef.w = w;
        sizeRef.h = h;
      ...
      onPointerMove = (e: MouseEvent) => {
        ...
        if (now - lastMoveTime < 16) return; // throttle pointer updates to 60fps
        lastMoveTime = now;
      ...
      const update = (now: number) => {
        ...
        const brightTarget = p.autoAnimate ? 1 : Math.max(0.7, proximityT);
        ...
        try {
          ...
          // graceful render catch
        }
      };
      raf = requestAnimationFrame(update);
    } catch (err) {
      console.warn('SpecularButton WebGL error:', err);
    }
