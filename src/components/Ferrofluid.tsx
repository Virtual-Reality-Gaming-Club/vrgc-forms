...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resolvedDpr = dpr ?? (isMobile ? 0.75 : Math.min(window.devicePixelRatio || 1, 1.25));

      renderer = new Renderer({
        dpr: resolvedDpr,
        alpha: true,
        antialias: false // fluid shader already applies soft noise/filtering; saves GPU memory & fillrate
      });
      ...
      const resize = () => {
        ...
        const w = Math.max(rect.width, window.innerWidth || 1920);
        const h = Math.max(rect.height, window.innerHeight || 1080);
        renderer.setSize(w, h);
        uniforms.iResolution.value = [gl.drawingBufferWidth || w, gl.drawingBufferHeight || h, 1];
      };
      ...
      const onPointerMove = (e: MouseEvent | PointerEvent) => {
        ...
        if (now - lastMoveTime < 16) return; // limit to ~60Hz pointer updates
        lastMoveTime = now;
      ...
      const loop = (t: number) => {
        ...
        if (programRef.current && meshRef.current) {
          try {
            ...
            // handle gl context loss gracefully
          }
        }
      };
      rafRef.current = requestAnimationFrame(loop);
      ...
      console.warn('WebGL / Ferrofluid initialization error:', err);
    }
  }, [
    dpr,
    paused,
