...
/* lightweight floating particles — pure CSS animated */
function Particles({ intensity }: { intensity: number }) {
  const particles = useMemo(() => {
    // 32 standard particles
...
/* Glowing energy orbs that drift around the screen */
...
/* Energy ring that expands on card pick and flip */
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none z-0" style={{ animation: 'camera-breathe 12s ease-in-out infinite' }}>
          ...
          {/* Central radial violet glow behind cards */}
          <motion.div
          ...
          {/* Hexagon pattern grid (replacing old perspective grid) */}
          ...
          {/* Card deck — takes full width for screen-spanning orbits */}
          ...
          {/* Welcome -> To -> VRGC Popup Sequence */}
          <WelcomePopup phase={phase} />

          {/* HUD overlay rendered before vignettes */}
          <HudOverlay phase={phase} />

          {/* Top DOF layer replacing vignette */}
          ...
          {/* Bottom DOF layer replacing vignette */}
