...
function computeOrbitalPositions(
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    // Per-card wobble adds organic feel
    ...
    const depthFactor = Math.sin(worldAngle); // -1 back, 1 front
    const y = Math.sin(worldAngle * 2 + 0.3) * radiusY * spread + wobbleY;

    // Chaos: some cards fly wider, some tighter
    const chaosScale = 1 + Math.sin(i * 1.4 + time * 3) * chaos * 0.3;

    const scale = (0.55 + (depthFactor + 1) * 0.3) * chaosScale; // 0.55→1.15
...
export default function CardDeck({ phase, onPhaseComplete, onMemberSelected }: CardDeckProps) {
  ...
  const [cardMembers, setCardMembers] = useState<Member[]>(() => MEMBERS.slice(0, CARD_COUNT));

  useEffect(() => {
    setCardMembers([...MEMBERS].sort(() => Math.random() - 0.5).slice(0, CARD_COUNT));
  }, []);
  ...
  // Responsive orbit dimensions — use large % of viewport
  ...
  useEffect(() => {
    const update = () => {
      ...
      setDims({
        rx: Math.min(vw * 0.42, 320),
        ry: Math.min(vh * 0.20, 160),
      });
    };
    update();
    ...
    switch (phase) {
      ...
      case 'CARD_PICK':
        if (selectedIndexRef.current === -1) {
          const pick = Math.floor(Math.random() * CARD_COUNT);
      ...
      {cardMembers.map((member, i) => (
        ...
        >
          <MemberCard
            ...
            isRevealed={
              i === selectedIndex &&
              ['AVATAR_REVEAL','PROFILE_EXPAND','COMPLETE'].includes(phase)
            }
          />
        </AnimatedCard>
      ))}
